#!/usr/bin/env python3
"""Factorial implementations: recursive and iterative with comments and basic tests.

This file provides two implementations of the factorial function:
- factorial_recursive(n): straightforward recursive implementation
- factorial_iterative(n): loop-based iterative implementation

Both functions validate input (must be non-negative integer) and raise
TypeError or ValueError for invalid input.

Run as a script to execute simple self-checks comparing both implementations
against Python's math.factorial.
"""
